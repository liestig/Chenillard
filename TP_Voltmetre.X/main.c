...
void initADC() {
    // Configurer l'entrée RA0 en tant qu'entrée analogique
    TRISAbits.TRISA0 = 1;  // RA0 en entrée

    // Activer le mode "left-justified" (le résultat de la conversion est aligné à gauche)
    ADCON2bits.ADFM = 1;

    // Configuration des tensions de référence VSS 0V et VDD 3V
    ADCON1bits.PVCFG0 = 0;
    ADCON1bits.NVCFG1  = 0;

    // Configure le résultat de la conversion pour une fréquence de fonctionnement de FOSC/64
    ADCON2bits.ACQT = 1;  // Acquisition time = 2TAD
    ADCON2bits.ADCS = 2;  // Clock conversion bits = FOSC/64

    // Configure le registre ADCON0 pour sélectionner l'entrée analogique RA0/AN0
    // et active le convertisseur ADC
    ADCON0bits.CHS = 0;    // Sélectionne l'entrée analogique RA0/AN0
...
void main(void) {
    ...
    while(1) {
        ...
        // Vérifier si la valeur a changé depuis la dernière itération
            // Effacer l'écran LCD
            LCDClear(); // Effacer l'écran
            LCDGoto(0, 0); // Positionner le curseur en haut à gauche
            LCDWriteStr("Tension: "); // Afficher le texte
            LCDWriteFloat(9,0 , voltage); // Afficher la valeur ADC
            LCDGoto(0, 14); // Positionner le curseur en haut à gauche
            LCDWriteStr("V");
