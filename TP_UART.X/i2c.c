 ...
 *
 * Modified by JMC to disable interrupts on START and STOP conditions
 * (SCIE and PCIE initialized to 0)
 */
...
void i2c_init(void) {
    ...
    SSP1CON3bits.SCIE = DISABLE;        // disable interrupt on start or restart condition (SCIE=0, bit 5)
    SSP1CON3bits.PCIE = DISABLE;        // disable interrupt on stop condition (PCIE=0, bit 6)
